...
> **Algofriend** is a production-grade, two-stage AI recommendation system. It combines deep metric learning for candidate retrieval (Two-Tower architecture + FAISS HNSW) with a sophisticated Learning-to-Rank (LTR) scoring mechanism (XGBoost) to deliver highly personalized, blazing-fast, scalable recommendations. ✨

---

## 🏗️ Architecture: The Two-Stage Pipeline 🌊

A single ML model cannot efficiently score a user against 10 million items in real-time. Algofriend solves this via the industry-standard two-stage funnel:
...
*   🏋️‍♂️ **The Training:** Trained using an **InfoNCE Contrastive Loss**, which learns to pull vectors of users and their interacted items closely together, while pushing non-interacted items apart.
...
*   ✅ **Async High-Performance API**: FastAPI with strict Pydantic validation, `asyncio.to_thread` for CPU-bound ranking, and `lru_cache` for fast embedding reuse.
*   ✅ **Background Indexing**: Non-blocking background tasks for dynamic FAISS index updating.
...
## 🔮 Roadmap 🗺️
- [ ] Connect `feast` (Feature Store) to fetch online point-in-time features during the API call. 🍽️
