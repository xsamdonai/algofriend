...
class UserEncoder(nn.Module):
    ...
    """
    def __init__(self, num_users=10000, user_feature_dim=20, embedding_dim=128):
...
class ItemEncoder(nn.Module):
    ...
    """
    def __init__(self, num_items=50000, item_feature_dim=50, embedding_dim=128):
...
class TwoTowerRecommendationModel(nn.Module):
    ...
    into the same dense semantic space. Distance (dot product) between 
    vectors correlates to interaction likelihood.
    """
    ...
    def encode_user(self, user_id: str) -> list:
        """
        ...
        """
        # In production: fetch features from Feast Redis Online Store
        self.eval()
        with torch.no_grad():
    ...
    def encode_item(self, item_id: str) -> list:
        self.eval()
        with torch.no_grad():
...
# -------------------------------------------------------------
# Contrastive Loss (InfoNCE) Concept for Training
# -------------------------------------------------------------
class InfoNCELoss(nn.Module):
    ...
    def forward(self, user_embs, item_embs):
        """
        user_embs: (Batch, Dim)
        item_embs: (Batch, Dim) - these are the POSITIVE items the user interacted with.
        
        The negative items are inherently all other items in the batch (In-Batch Negatives).
        """
